...
get_sources(){
    ...
    if [ ${NO_CLONE} = 0 ]; then
        ...
        for dir in 'percona-xtradb-cluster-galera'; do
            cd $dir || exit
            git submodule deinit -f . || true
            git submodule init
            git submodule update
    ...
    WSREP_VERSION="$(grep WSREP_INTERFACE_VERSION wsrep/src/wsrep_api.h | cut -d '"' -f2).$(grep 'WSREP_VERSION_PATCH'  WSREP_VERSION | awk -F'=' '{print $2}')"
    ...
    echo "XB_VERSION=${XB_VERSION}" >> ${WORKDIR}/pxc-57.properties
    ...
    echo "DESTINATION=UPLOAD/${DESTINATION}/BUILDS/${PRODUCT}/${PRODUCT_FULL}/${BRANCH}/${REVISION}/${BUILD_NUMBER}" >> ${WORKDIR}/pxc-57.properties
    ...
    if [ -f /etc/redhat-release ]; then
      ...
      RHEL=$(rpm --eval %rhel)
      if [ "x${RHEL}" = "x8" ]; then
          cmake . -DDOWNLOAD_BOOST=1 -DWITH_ROCKSDB=0 -DWITH_BOOST=build-ps/boost -DWITH_WSREP=1
      else
          cmake3 . -DDOWNLOAD_BOOST=1 -DWITH_ROCKSDB=0 -DWITH_BOOST=build-ps/boost -DWITH_WSREP=1
      fi
    else
      cmake . -DDOWNLOAD_BOOST=1 -DWITH_ROCKSDB=0 -DWITH_BOOST=build-ps/boost -DWITH_WSREP=1
    fi
    ...
    rsync -av ${WORKDIR}/percona-xtradb-cluster/wsrep/ ${PXCDIR}/wsrep --exclude .git
...
#                /usr/bin/pip2.7 install --user typing pyyaml regex Cheetah
        ...
        apt-get -y install curl bison make cmake perl libssl-dev gcc g++ libaio-dev libz-dev libc-dev libldap2-dev libwrap0-dev gdb unzip gawk
        apt-get -y install lsb-release libmecab-dev libncurses-dev libncurses5-dev libreadline-dev libpam-dev zlib1g-dev libcurl4-gnutls-dev
        ...
        apt-get -y install libtool libnuma-dev scons libboost-dev libboost-all-dev libboost-program-options-dev check
...
build_srpm(){
    ...
    sed -i "s:@@XB_VERSION@@:${XB_VERSION}:g" rpmbuild/SPECS/percona-xtradb-cluster.spec
...
build_rpm(){
    ...
    if test "x${SCONS_ARGS}" == "x"
    then
        rpmbuild --define "_topdir ${WORKSPACE}/rpmbuild" --define "dist el${RHEL}" --define "galera_revision ${GALERA_REVNO}" --define "rpm_version ${RPM_RELEASE}" --define "with_mecab ${MECAB_INSTALL_DIR}/usr" --rebuild rpmbuild/SRPMS/${SRCRPM}
    else
        rpmbuild --define "_topdir ${WORKSPACE}/rpmbuild" --define "dist el${RHEL}" --define "galera_revision ${GALERA_REVNO}" --define "rpm_version ${RPM_RELEASE}" --define "scons_args ${SCONS_ARGS}" --define "with_mecab ${MECAB_INSTALL_DIR}/usr" --rebuild rpmbuild/SRPMS/${SRCRPM}
    fi
    return_code=$?
...
#    NEWTAR=${NAME}_${VERSION}-${RELEASE}.orig.tar.gz
    ...
    #
    tar xzf ${NEWTAR}
    cd ${HNAME}-${VERSION}-${WSREP_VERSION} || exit
...
build_deb(){
    ...
    cd ${DIRNAME} || exit
    if [[ "x$DEBIAN_VERSION" == "xbionic" || "x$DEBIAN_VERSION" == "xfocal" || "x$DEBIAN_VERSION" == "xbuster" || "x$DEBIAN_VERSION" == "xcosmic" || "x$DEBIAN_VERSION" == "xyakkety" || "x$DEBIAN_VERSION" == "xstretch" || "x$DEBIAN_VERSION" == "xbullseye" || "x$DEBIAN_VERSION" == "xjammy" ]]; then
    ...
    if [ ${DEBIAN_VERSION} = xenial -o ${DEBIAN_VERSION} = stretch -o ${DEBIAN_VERSION} = bionic -o ${DEBIAN_VERSION} = focal -o ${DEBIAN_VERSION} = buster -o ${DEBIAN_VERSION} = bullseye -o ${DEBIAN_VERSION} = jammy ]; then
        rm -rf debian
    ...
    if [ x"${DEBIAN_VERSION}" = xbionic -o x"${DEBIAN_VERSION}" = xcosmic -o x"${DEBIAN_VERSION}" = xbuster -o x"${DEBIAN_VERSION}" = xfocal -o x"${DEBIAN_VERSION}" = xbullseye -o x"${DEBIAN_VERSION}" = xjammy ]; then
